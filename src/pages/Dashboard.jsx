...
const Dashboard = () => {
    ...
    const createLead = async () => {
        try {
            ...
            const res = await axios.post("http://localhost:3000/createlead", {
                name,
                email,
            });
    ...
    const sendEmail = async (id) => {
        try {
            const res = await axios.post(
                `http://localhost:3000/sendEmail/${id}`
            );
            // res.data.msg ||
    ...
    const deletelead = async (id) => {
        try {
            await axios.delete(
                `http://localhost:3000/deletelead/${id}`
            );
    ...
    const updateStatus = async (id, status) => {
        await axios.put(`http://localhost:3000/updatelead/${id}`, { status });
        getLeads();
    };
    ...
    const viewLead = async (id) => {
        const res = await axios.get(`http://localhost:3000/leads/${id}`);
        const actRes = await axios.get(`http://localhost:3000/activities/${id}`);
